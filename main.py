...
def index():
    ...
    record = util.run_and_fetch_sql(cursor, "SELECT a, fruit_a, b, fruit_b FROM basket_a FULL OUTER JOIN basket_b ON basket_a.fruit_a = basket_b.fruit_b WHERE basket_a.fruit_a IS NULL OR basket_b.fruit_b is NULL;")
    if record == -1:
        # you can replace this part with a 404 page
        print('Something is wrong with the SQL command')
    else:
